...
const func: DeployFunction = async({getNamedAccounts, deployments, network, ethers}) => {
  ...
  const args = [
    process.env.MAINNET_TREASURY, //_treasury
    process.env.MAINNET_TREASURY, //_keeper
    '0x51e44FfaD5C2B122C8b635671FCC8139dc636E82', // _usdc - when mainnet need to modify it.
    ...
    parseUnits("22220", 18), // _minTokensAmount -> $999 / 0.045 = 22220
    parseUnits("444445", 18), // _maxTokensAmount -> $20000 / 0.045 = 444445
    parseUnits("17333335", 18), // _privateSaleTokenPool -> 14,000,000
    Date.UTC(2022, 4, 6, 15, 0, 0) / 1000, // _privateSaleStart year: 2000, month: 0, date: 1, hour: 12, minute: 34, second: 5
    Date.UTC(2022, 4, 8, 15, 0, 0) / 1000, // _privateSaleEnd year: 2000, month: 0, date: 1, hour: 12, minute: 34, second: 5
